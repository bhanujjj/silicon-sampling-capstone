"""Pew Track: QLoRA fine-tune on real Pew "Religion in India" (2021)
respondents, out-of-fold -- the Pew-dataset counterpart to
scripts/trackB_finetune.py (WVS-7). Same hardened one-shot-run design
(preflight, auto-resume, OOM backoff, status file, signal handler),
same prompt/parsing pipeline shape, swapped to:

    - data: data/processed/pew_india_2021.parquet (29,999 respondents,
...
it auto-detects the latest checkpoint under pewB_run/checkpoints/ and
resumes from there instead of starting over.

EVERYTHING THIS SCRIPT WRITES LIVES UNDER ONE FOLDER (--output-dir, default
./pewB_run), so the whole run -- data cache, checkpoints, logs, live
...
if you need to move it off the lab machine:
    ...
    pewB_run/
      ...
      logs/
        pewB_run.log            <- full human-readable log, this run only
      ...
      predictions/
        ...
        pewB_<model>_fold<N>_P2.parquet   <- final predictions (also
                                   copied to results/predictions/ at repo
                                   root, unchanged, for src.report.*)
      model_fold_<N>/     <- final saved LoRA adapter + tokenizer (see
                           scripts/pewB_inference.py to load this back up
                           and answer new questions WITHOUT retraining)

pewB_status.json at the REPO ROOT (not inside pewB_run/) mirrors
trackB_status.json's role -- `cat pewB_status.json` from another pane.
"""
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("pewB_run.log")],
)
...
def preflight(args) -> bool:
    ...
    if total_mem_gb < MIN_GPU_MEM_GB:
        logger.error(f"PREFLIGHT FAIL: only {total_mem_gb:.0f} GB total GPU memory visible -- too little for {DEFAULT_MODEL} even in 4-bit (want at least {MIN_GPU_MEM_GB} GB). Check you got the GPU allocation you expected.")
        ok = False
    ...
    if n_selected == 0:
        logger.error(
            ...
            "wording -- see src/data/build_pew_codebook.py's docstring. Parse "
            "Pew's own codebook document (scripts/parse_pew_recode_syntax.py "
            "for the recode-syntax .txt, or by hand), re-run "
            "`python -m src.data.build_pew_codebook --labels <file>`, then "
            "`python -m src.data.select_items_pew`, before continuing."
        )
        return False
    ...
    logger.info("=" * 60)
    logger.info("PREFLIGHT PASSED. Next: python -m scripts.pewB_finetune --smoke-test")
    logger.info("=" * 60)
    return True
...
def main():
    ...
    if not selected_items:
        logger.error(
            ...
            "wording yet. Run --preflight first; it explains exactly what's "
            "missing and how to fix it. Refusing to build a training set out "
            "of unlabeled columns."
        )
        sys.exit(1)
        ...
        for question_id in selected_items:
            try:
                ...
                rows.append({
                    ...
                    "condition": "P2",
                    "model": f"pewB_{args.model.replace('/', '_')}_fold{args.fold}",
                    "true_code": true_code,
